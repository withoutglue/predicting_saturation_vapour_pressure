...
We went on to explore the data first, carry out feature engineering, try out different models that we learned during the course, evaluate our models performance on chosen metrics and finally perform model selection with what we would take part in this Kaggle competition.
...
When exploring linearity between response and each variable we fitted a linear model per each and looked at the residual plots. NumOfConf was the variable that seemed to have the least linear relationship between the response. You can see from the plot (Figure 1. lower plot) that residuals has this funnel-like shape that indicates non-linearity and non-constant variance in relation.
...
Inspecting correlation matrix in Figure 1 we see that there is strong correlation between variables in top left corner. This is logical since all variables correlating heavily are related to number of atoms and molecular weight. We take this into consideration in feature engineering.

We also inspected the variables using the Principal Component Analysis (PCA). Figure 5 shows the PCA plot. From it, we can inspect, that the first 2 principal components explain around 28% of the total variability of the response. We also see, that quite some number of the variables add little to none explaining power to the variance. We explored this further in subset selection for linear regression.
...
Based on the residual plots we decided to try different transformations (exp, log, different order of polynomials) with NumOfConf. From the residual plot in Figure 2 we can see that this transformation truly affects the residuals to appear more randomly than without it. This indicates that this relation with log-transformation is more linear. In practice it increased our score a little bit with linear models so we decided to experiment this transformation with other models as well.
...
Next we decided to reduce most of the highly correlated variables. We ended up removing following variables from our data set: MW, NumOfAtoms, NumHBondDonors and NumOfConf. Correlation matrix for this subset can be seen in Figure 3.

```{r feature engineering, echo=FALSE, fig.cap= "Corralation matrix with removed variables to reduce correlation between variables."}
...
As we can see this correlation matrix more neutral in terms of correlation. Next we inspected how does this removal of variables compare in subset selection. We got following scores using reduced data set and full data set:

```{r, warning=FALSE, echo=FALSE, message=FALSE, fig.cap= "Left curve is with reduced data set and right is with full data set."}
regfit.full <- regsubsets(log_pSat_Pa ~., data = kag_train_processed, nvmax = 21)
...
regfit.full <- regsubsets(log_pSat_Pa ~., data = kag_train, nvmax = 21)
reg.summary <- summary(regfit.full)
#reg.summary$rsq
...
As we can see from figure 4 removing these variables just to remove correlation within our data set does not improve adjusted $R^2$ significantly. Because of this observation we decided to continue our feature selection with full data set just in case we lack some domain knowledge and accidentally remove some variables of whose effect on our model performance we can not guess.

Next we did feature selection with forward and backward step wise methods and received following results:
...
regfit.fwd <- regsubsets(log_pSat_Pa ~., data = kag_train, nvmax = 21, method = "forward")
fwd.summary <- summary(regfit.fwd)
regfit.bwd <- regsubsets(log_pSat_Pa ~., data = kag_train, nvmax = 21, method = "backward")
...
Because forward selection (Figure 5) seems to explain variance a little bit better in the beginning and it seems to plateu at roughly 13 variables we decided to take this sub set and try it with our models. This sub set is as follows:
...
From this point on we decided to try following models and see how they perform against out baseline; linear model with feature selection and engineering, PCR, RF and tree based methods, and SVM. In the following sections we describe these models in more detail.
...
We decided to try linear model with this feature engineered sub set with and without log-transformation of NumOfConf. We got following results with feature engineered subset:
...
We got following results with feature engineered subset with log transformation on NumOfConf:
...
When we tried PCR with full data set excluding ID we got following results:
...
```{r, echo=FALSE, fig.cap= "R2 scores with PRC on full data set."}
validationplot(pcr.fit, val.type = "R2")
```


PCR model reaches score 70.61 with 24 components and with 27 components it reaches it's max score 70.77.
...
library(knitr)
kable(scores, caption="Private and public scores of the most important models")
```
